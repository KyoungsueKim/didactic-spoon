...
uint8_t SC16IS752Serial::getFIFOStatus() {
    // Read both TX and RX FIFO levels
    uint8_t txLevel = readRegister(_address, _channel, SC16IS7XX_TXLVL_REG);
    uint8_t rxLevel = readRegister(_address, _channel, SC16IS7XX_RXLVL_REG);

    // Also check FCR register to verify FIFO is enabled
    uint8_t fcr = readRegister(_address, _channel, SC16IS7XX_FCR_REG);

    // If we can't read the registers or FIFO is disabled, return 0xFF (error)
    if ((txLevel == 0 && rxLevel == 0 && fcr == 0) || !(fcr & SC16IS7XX_FCR_FIFO_BIT)) {
        return 0xFF;
    }

    // Return the maximum of TX and RX levels to indicate FIFO is working
    return (txLevel > rxLevel) ? txLevel : rxLevel;
}
